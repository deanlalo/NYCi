...
export function generatePDF(state, prices, company, headerImage) {
  const doc = new jsPDF();
  ...
  function addHeader() {
    if (headerImage) {
      try {
        ...
        doc.text(company || 'Low Voltage Contractor', m, 20);
      }
    } else {
      doc.setFontSize(14);
      doc.setFont('helvetica', 'bold');
    ...
    doc.text('PROPOSAL', pw - m, 14, { align: 'right' });
    doc.text(new Date().toLocaleDateString('en-US'), pw - m, 20, {
      align: 'right',
    });
    doc.setTextColor(0);
  }

  /* ── Footer helper (every page) ────────── */
  ...
  doc.setFont('helvetica', 'normal');
  const contactStr = [state.project.contactName, state.project.contactPhone]
    .filter(Boolean)
    .join(' — ') || 'N/A';
  ...
  doc.text('Date:', m, y);
  doc.setFont('helvetica', 'normal');
  doc.text(new Date().toLocaleDateString('en-US'), m + 16, y);
  y += 16;
